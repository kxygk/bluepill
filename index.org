...
** CMake
CMake will be our central tool for coordinating the build and we will define all of it in a ~CMakeLists.txt~ file in our project root. Starting our CMake file we write out the usual CMake version and project name
...
Note that for the configuration file to run register with  =openocd= when run in the shell it needs to be saved in a file called ~openocd.cfg~ 
