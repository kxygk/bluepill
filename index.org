...
So because we can't just start at address zero and run foward or drop in a ~main()~ randomly and start there, we need to tell the linker what the code layout is though a custom *linker script*
...
Even give that, code on a microcontroller doesn't start at the first address of ROM and go from there (though you can make it kinda work that way). The way it works is that certain external factors generate interrupt signals and the chip responds by running code and then returning to whatever it was doing before. These factors can be a clock running out, a peripheral wanting attention, an attached debugger wanting a halt, etc.
...
EXTERN(vector_table);
#+END_SRC
~vector_table~ is just not a special keyword, it's just a label we are giving the linker. It's written as =EXTERN=, meaning it'll look for its definition in our code, so we need to not forget to define this later on!

The first interrupt in the table is special and it's the one that is generated when the system is powered on, the user presses the reset button, or the code runs out of things to do. The code that it triggers is appropriately called the *reset handeler*

That the code should start at the *reset handler*
#+BEGIN_SRC c :tangle STM32F103RBTx_FLASH.ld
ENTRY(reset_handler);
#+END_SRC
Again, this is just a symbol and the linker will look for its value in our code during linking.

Lastly we just need to tell the linker that we want those vectors first in the ROM and aligned at the byte level (b/c the micro reads things in byte sized chucks - haha)
#+BEGIN_SRC c :tangle STM32F103RBTx_FLASH.ld
SECTIONS
{
    .text : {
...
TODO: Copied from elsewhere... startup needs to be pulled out of =satoshinm='s main.c
